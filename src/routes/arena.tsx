...
function ArenaHostPage() {
  ...
  const onCreate = async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : "開房失敗");
    } finally {
      setBusy(false);
    }
  };
  ...
  const onNext = async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : "換題失敗");
    } finally {
      setBusy(false);
    }
  };
      ...
      {!hydrated ? null : !session ? (
        <Card>
          <CardContent className="flex flex-col items-start gap-4 p-6">
            <p className="text-ink-soft">還沒有房間。按下面按鈕開一場新的吧！</p>
        ...
        <div className="space-y-5">
          ...
          <div className="flex flex-wrap gap-2">
            {phase === "lobby" ? (
              <Button size="lg" onClick={onStart} disabled={busy || !state?.players.length}>
                開始比賽
              </Button>
            ) : null}
            {phase === "question" ? (
          ...
          {phase === "lobby" ? (
            <PlayerList players={state?.players ?? []} empty="等同學掃碼加入…" />
          ) : null}
                    ...
                    return (
                      <div
                        ...
                        className={cn(
                          ...
                          isCorrect
                            ? "border-leaf bg-leaf/20 text-ink"
  ...
  return (
    <ul className="divide-y divide-ink/8 overflow-hidden rounded-xl border-2 border-ink/8 bg-card">
      {players.map((p, idx) => (
        <li
          key={p.id}
          className="flex items-center justify-between gap-3 px-4 py-3 font-display"
        >
