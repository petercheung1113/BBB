...
import { Castle, Compass, Shapes, Sparkles, Star, Trophy } from "lucide-react";
...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  const stars = totalStars(lessons, quizBest, fractions ?? {});
    ...
    <div className="relative min-h-dvh overflow-x-hidden bg-sky text-ink">
      ...
      <header className="relative z-20 border-b-2 border-ink/8 bg-paper/80 backdrop-blur-md">
        <div className="mx-auto flex h-16 max-w-5xl items-center justify-between gap-3 px-4">
          ...
          <nav className="hidden items-center gap-1 lg:flex">
            ...
            <NavLink to="/castles/shapes">圖形城堡</NavLink>
            <NavLink to="/castles/fractions">分數城堡</NavLink>
      ...
      <nav className="fixed inset-x-0 bottom-0 z-30 border-t-2 border-ink/8 bg-paper/95 pb-[env(safe-area-inset-bottom)] backdrop-blur-md sm:hidden">
        <div className="mx-auto grid max-w-lg grid-cols-5 px-1 py-2">
          ...
          <Tab to="/castles/shapes" icon={<Shapes className="size-5" />} label="圖形" />
          <Tab to="/castles/fractions" icon={<Castle className="size-5" />} label="分數" />
